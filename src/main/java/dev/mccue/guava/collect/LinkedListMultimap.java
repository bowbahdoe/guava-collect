 ...
 * }</pre>
 *
 * ... the iteration order for {@code #keys()} is {@code [key1, key2, key1]}, and similarly for
 * {@code #entries()}. Unlike {@code LinkedHashMultimap}, the iteration order is kept consistent
 ...
 * order to {@code [key2, key1]}. The {@code #entries()} iterator returns mutable map entries, and
 * {@code #replaceValues} attempts to preserve iteration order as much as possible.
 *
 * <p>The collections returned by {@code #keySet()} and {@code #asMap} iterate through the keys in
 * the order they were first added to the multimap. Similarly, {@code #get}, {@code #removeAll}, and
 * {@code #replaceValues} return collections that iterate through the values in the order they were
 * added. The collections generated by {@code #entries()}, {@code #keys()}, and {@code #values}
 * iterate across the key-value mappings in the order they were added to the multimap.
 *
 * <p>The {@code #values()} and {@code #entries()} methods both return a {@code List}, instead of
 * the {@code Collection} specified by the {@code ListMultimap} interface.
 *
 * <p>The methods {@code #get}, {@code #keySet()}, {@code #keys()}, {@code #values}, {@code
 * #entries()}, and {@code #asMap} return collections that are views of the multimap. If the
...
public class LinkedListMultimap<K extends @Nullable Object, V extends @Nullable Object>
   ...
   * the values for a specific key, is used to implement {@code
   ...
   * Multimap}. The new multimap has the same {@code Multimap#entries()} iteration order as the
   * input multimap.
   *
   ...
   * the {@code Iterator} classes. See also {@code LinkedListMultimap#removeAllNodes(Object)}.
   */
   ...
   *
   * <p>The returned list is immutable and implements {@code java.util.RandomAccess}.
   */
  @CanIgnoreReturnValue
  @Override
   ...
   * ordering, this method returns a {@code List}, instead of the {@code Collection} specified in
   * the {@code ListMultimap} interface.
   */
  @Override
   ...
   * the {@code ListMultimap} interface.
   *
   * <p>An entry's {@code Entry#getKey} method always returns the same key, regardless of what
   * happens subsequently. As long as the corresponding key-value mapping is not removed from the
   * multimap, {@code Entry#getValue} returns the value from the multimap, which may change over
   * time, and {@code Entry#setValue} modifies that value. Removing the mapping from the multimap
