...
/**
 * A {@code Multimap} whose contents will never change, with many other important properties
 * detailed at {@code ImmutableCollection}.
 *
 * <p><b>Warning:</b> avoid <i>direct</i> usage of {@code ImmutableMultimap} as a type (as with
 * {@code Multimap} itself). Prefer subtypes such as {@code ImmutableSetMultimap} or {@code
 * ImmutableListMultimap}, which have well-defined {@code #equals} semantics, thus avoiding a common
 * source of bugs and confusion.
 *
 * <p><b>Note:</b> every {@code ImmutableMultimap} offers an {@code #inverse} view, so there is no
 ...
 * together. Any creation method that would customarily respect insertion order (such as {@code
...
public abstract class ImmutableMultimap<K, V> extends BaseImmutableMultimap<K, V>
  ...
  public static class Builder<K, V> {
     ...
     * {@code #expectedValuesPerKey} as its hint.
     *
